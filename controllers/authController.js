...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (!user || !userChecked) return next(new AppError("Email or Password is wrong!", 401));
...
exports.restrictTo = (...roles) => {
    return (req, res, next) => {
        ...
        if (!roles.includes(req.user.role)) return next(new AppError("Insufficient access!", 403));
        next();
    };
};
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        return next(new AppError("There was an error in sending the email! Try Again.", 500));
    }
});
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) return next(new AppError("Token has been expired or invalid token!", 400));
    ...
    // WE HAVE MIDDLE FOR SAVING THIS IN USERSCHEMA
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    const userChecked = await user?.checkPassword(req.body.currentPassword, user.password);
